...
pub const FRAGMENT_SHADER_120: &'static str = "
    ...
    void main() {
        ...
        // 2D Geometry
            ...
            vec3 v_normal = vec3(v_tex_coords.x,v_tex_coords.y,1.0);
            lowp float lum = max(dot(normalize(v_normal), normalize(LIGHT)), 0.0);
            gl_FragColor = (0.3+0.7*lum)*v_color;
        }
    }
";
...
pub fn text_texture_uncompressed_float_format(opengl_version: &glium::Version)
...
impl GlyphCache {
        ...
        where F: glium::backend::Facade
    {
        ...
        let texture =
            try!(glium::texture::Texture2d::with_format(facade, grey_image, format, no_mipmap));
            ...
            match kind {
                ...
                render::PrimitiveKind::Text { color, text, font_id } => {
                    ...
                    let to_gl_rect = |screen_rect: text::rt::Rect<i32>| {
                        text::rt::Rect {
                            min: origin +
                                 (text::rt::vector(screen_rect.min.x as f32 / screen_w as f32 -
                                                   0.5,
                                                   1.0 -
                                                   screen_rect.min.y as f32 / screen_h as f32 -
                                                   0.5)) * 2.0,
                            max: origin +
                                 (text::rt::vector(screen_rect.max.x as f32 / screen_w as f32 -
                                                   0.5,
                                                   1.0 -
                                                   screen_rect.max.y as f32 / screen_h as f32 -
                                                   0.5)) * 2.0,
                        }
                    };
                    ...
                    for g in positioned_glyphs {
                        if let Ok(Some((uv_rect, screen_rect))) = cache.rect_for(cache_id, g) {
                            ...
                            push_v([gl_rect.min.x, gl_rect.max.y],
                                   [uv_rect.min.x, uv_rect.max.y]);
                            push_v([gl_rect.min.x, gl_rect.min.y],
                                   [uv_rect.min.x, uv_rect.min.y]);
                            push_v([gl_rect.max.x, gl_rect.min.y],
                                   [uv_rect.max.x, uv_rect.min.y]);
                            push_v([gl_rect.max.x, gl_rect.min.y],
                                   [uv_rect.max.x, uv_rect.min.y]);
                            push_v([gl_rect.max.x, gl_rect.max.y],
